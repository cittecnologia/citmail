...
- CI: `.github/workflows/testes.yml` roda três jobs em todo PR para `develop` e `main`, todos exigidos para o merge: `playwright` (`npm test`), `api` (`npm --prefix api run lint` e `npm --prefix api test` com PostgreSQL de serviço) e `segredos` (gitleaks, versão e SHA-256 fixados no workflow, com as regras de `.gitleaks.toml`, só nos commits do PR). É reaproveitado pela homologação via `workflow_call` (lá `segredos` fica pulado). Actions fixadas por SHA com comentário da versão.
- Scan de segredos local, antes do push: `gitleaks git --config .gitleaks.toml --redact --log-opts="origin/develop..HEAD"` (esperado: `no leaks found`). Achado real não se corrige com allowlist: vira issue com rotação da credencial.
