...
- **Atributos:**
  ...
  - Domínio: `dominio_nome`, `dominio_tipo`, `dominio_verificado_em` (E4-H3). Domínio ainda não é entidade própria; pode virar na #53 ou depois da #48.
...
- **Token de senha (E5-H2):** `token_senha_hash`, `token_senha_tipo` (`primeiro_acesso` ou `redefinicao`), `token_senha_expira_em`, `token_senha_usado_em`. Só o hash do token é gravado. Tabela própria ou colunas: decisão da #53.
- **Relações:** pertence a 1 cliente; administra N caixas; aparece na auditoria como autor ou alvo.
...
- **Atributos:** `nome`, `versao`, `agregado_tipo` (`pedido`, `conta`, `assinatura`, `dominio`), `agregado_id`, `payload` (JSON só com ids e dados não pessoais), `request_id` (correlação, ADR 0013), `ocorrido_em`, `publicado_em` (nulo até ir para a fila), `tentativas_publicacao`.
- **Relações:** pertence a 1 agregado (pedido, conta do titular ou assinatura).
- **Regras:** a restrição única em `chave_idempotencia` impede o mesmo evento duas vezes. O webhook bruto do Asaas (E3-H4) e o log de chamadas à Skymail (E4-H1) ficam em tabelas próprias, fora deste anexo.
...
| `pago` | webhook de pagamento da fatura das contas (a página mostra "ativando") | E3-H4, E3-H5 |
| `aguardando_registro_de_dominio` | `dominio_pago` com domínio novo; tarefa manual aberta | E4-H4 |
...
stateDiagram-v2
  ...
  [*] --> falhou_na_criacao
  aguardando_pagamento --> pago: pedido_pago
  pago --> ativo: provisionamento ok
  pago --> aguardando_registro_de_dominio: dominio_pago (domínio novo)
...
- `aguardando_pagamento` → `pago`: só uma vez por pedido, mesmo com webhook repetido (E3-H4).
- `pago` → `ativo`: domínio existente, pelo job de E4-H2.
- `pago` → `aguardando_registro_de_dominio` → `pago`: domínio novo. A equipe marca o registro e o job de E4-H2 é enfileirado (E4-H4).
...
- A definir: expiração de pedido `aguardando_pagamento` nunca pago, e se `falhou_na_criacao` permite nova tentativa no mesmo pedido.
